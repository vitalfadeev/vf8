module vf.sdl.renderer_thorvg;

version (THORVG):
import std.conv : to;
import vf.sdl.importc : 
    SDL_Window,SDL_Surface,Uint32,SDL_GetWindowSurface,
    SDL_PIXELFORMAT_ABGR8888,SDL_PIXELFORMAT_RGB888, SDL_PIXELFORMAT_ARGB8888,
    SDL_GetPixelFormatName,SDL_GetWindowFromID,
    SDL_UpdateWindowSurface,SDL_FreeSurface;
import vf.sdl.importc_tvg;
...
Renderer {
    ...
    new_surface (SDL_Window* window) {
        ...
        switch (sformat) {
            ...
            default:
                throw new Exception ("unknown pixel format: " ~ SDL_GetPixelFormatName (sformat).to!string);
        }
    ...
    draw_start (O,Event) (O o, Event* evt) {
        SDL_Window* window = SDL_GetWindowFromID (evt.sdl.window.windowID);
